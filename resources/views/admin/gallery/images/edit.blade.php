...
@section('content')
	<div class="max-w-3xl p-4 mx-auto">
		<h1 class="mb-4 text-xl font-semibold">Edit Gallery Image</h1>
		@if ($errors->any())
				...
				<div>
					...
					<select name="category_id" class="w-full p-2 bg-white border rounded">
						@foreach($categories as $id => $name)
							<option value="{{ $id }}" {{ old('category_id', $image->category_id)==$id?'selected':'' }}>{{ $name }}</option>
						@endforeach
					</select>
				</div>
				<div>
					...
					@if($image->image)
						<p class="mt-1 text-sm text-gray-600">Current: {{ $image->image }}</p>
					@endif
				</div>
				<div>
					<label class="block mb-1 text-sm font-medium">Title (optional)</label>
					<input type="text" name="title" value="{{ old('title', $image->title) }}" class="w-full p-2 border rounded">
				</div>
			</div>
