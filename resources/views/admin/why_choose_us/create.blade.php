...
@section('content')
	<div class="max-w-2xl p-4 mx-auto">
		<h1 class="mb-4 text-xl font-semibold">Add Why Choose Us Item</h1>
		@if ($errors->any())
		...
		<form action="{{ route('admin.why-choose-us.store') }}" method="POST" enctype="multipart/form-data">
			...
			<div class="grid grid-cols-1 gap-4">
				...
				<div>
					...
					<textarea name="description" rows="5" class="w-full p-2 border rounded">{{ old('description') }}</textarea>
				</div>
				<div>
				...
				<div>
					...
					<input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" class="w-full p-2 border rounded">
				</div>
			</div>
