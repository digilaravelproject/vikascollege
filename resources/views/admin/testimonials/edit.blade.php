...
@section('content')
	<div class="max-w-2xl p-4 mx-auto">
		<h1 class="mb-4 text-xl font-semibold">Edit Testimonial</h1>
		@if ($errors->any())
		...
		<form action="{{ route('admin.testimonials.update', $testimonial) }}" method="POST" enctype="multipart/form-data">
			...
			<div class="grid grid-cols-1 gap-4">
				<div>
					...
					<input type="text" name="student_name" value="{{ old('student_name', $testimonial->student_name) }}" class="w-full p-2 border rounded">
				</div>
				<div>
					...
					@if($testimonial->student_image)
						<p class="mt-1 text-sm text-gray-600">Current: {{ $testimonial->student_image }}</p>
					@endif
				</div>
				<div>
					<label class="block mb-1 text-sm font-medium">Testimonial</label>
					<textarea name="testimonial_text" rows="5" class="w-full p-2 border rounded">{{ old('testimonial_text', $testimonial->testimonial_text) }}</textarea>
				</div>
				<label class="inline-flex items-center gap-2">
					<input type="checkbox" name="status" value="1" class="rounded" {{ old('status', $testimonial->status) ? 'checked' : '' }}>
					<span>Approved</span>
				</label>
			</div>
