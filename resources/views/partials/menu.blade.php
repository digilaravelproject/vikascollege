                    ...
                    @foreach ($menus as $menu)
                        <li x-data="{ openSub: false, activeLeftIndex: 0 }" class="relative group">
                            @if ($menu->children->count())
                                <a href="{{ $menu->url }}" @mouseenter="openSub = true" @mouseleave="openSub = false"
                                  ...
                                  {{ Request::is(trim(parse_url($menu->url, PHP_URL_PATH), '/')) ? 'after:w-full text-[#35abe7]' : 'after:w-0 hover:after:w-full' }}">
                                    {{ $menu->title }}
                                    <svg class="w-4 h-4 text-gray-100 transition group-hover:text-gray-300"
                                    ...
                                    <div x-show="openSub" @mouseenter="openSub = true" @mouseleave="openSub = false"
                                        ...
                                        <div
                                            ...
                                            <div class="flex">
                                                ...
                                                <div class="w-2/3 p-6">
                                                    @foreach ($menu->children as $index => $leftItem)
                                                        <div x-show="activeLeftIndex === {{ $index }}" x-cloak
                                                            x-transition class="space-y-4">
                                                            @php$subs = $leftItem->children ?? collect();
                                                                $count = $subs->count();
                                                            @endphp
                                        ...
                                        <div x-show="openSub" @mouseenter="openSub = true" @mouseleave="openSub = false"
                                            ...
                                            <ul class="flex flex-col py-1 min-w-[220px]">
                                                @foreach ($menu->children as $child)
                                                    <!-- each child has its own Alpine state so sub-children show only when THIS child is hovered -->
                                                    <li x-data="{ openChild: false }" @mouseenter="openChild = true"
                                                        ...
                                                        <a h ref="{{ $child->url }}"
                                                            ...
                                                            @if ($child->children->count())
                                                                <!-- small right arrow for items with sub-children -->
                                                        ...
                                                        <!-- Sub-Child Menu (visually distinct) -->
                                                        @if ($child->children->count())
                                                                ...
                                                                @foreach ($child->children as $subchild)
                                                                    <li>
                                                                        <a href="{{ $subchild->url }}"
                                                                            ...
                                                                            <spa n
                                                                                class="inline-block pl-1 border-l-2 border-[#35abe7]">
                                                                                {{ $subchild->title }}</span>
                                                                        </a>
                                                                    </li>
                                                                @endforeach
                            ...
                            <div x-show="openSub" x-transition class="bg-gray-50">
                                @foreach ($menu->children as $child)
                                    ...
                                    @if ($child->children->count())
                                        @foreach ($child->children as $subchild)
                                            <a href="{{ $subchild->url }}"
                                                class="block px-12 py-2 text-xs text-gray-600 hover:bg-blue-50">
                                                — {{ $subchild->title }}
                                            </a>
                                        @endforeach
                                    @endif
